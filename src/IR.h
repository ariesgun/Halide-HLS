...
inline bool _is_zero(const Expr& e) {
    if (const FixedPointImm *fixed_imm = e.as<FixedPointImm>()) return fixed_imm->value.i == 0;
    return 0;
}
...
struct Add : public ExprNode<Add> {
    ...
    inline Type match_output_type(const Expr &a, const Expr &b) {
        ...
        if (ta.is_fixed_ufixed_point() && tb.is_float()) {
            ...
            // Convert to float
            return ta;
        } else if (ta.is_fixed_ufixed_point() && (tb.is_uint() || tb.is_int())) {
            ...
            //out_type = 
        } else if ((ta.is_uint() || ta.is_int()) && tb.is_fixed_ufixed_point()) {
...
struct Sub : public ExprNode<Sub> {
    ...
    inline Type match_output_type(const Expr &a, const Expr &b) {
        ...
        if (ta.is_fixed_ufixed_point() && tb.is_float()) {
            ...
            int bits_a = 0;
            if (int_a) {
                bits_a = get_min_nb_bits(int_a >= 0 ? int_a->value : std::abs(int_a->value) + 1);
            } else if (uint_a) {
                bits_a = get_min_nb_bits(uint_a->value);
            } 
            return FixedPoint(std::max(tb.int_bits(), bits_a) + std::max(tb.bits() - tb.int_bits(), 0), std::max(tb.int_bits(), bits_a), std::max(ta.lanes(), tb.lanes()));
            //return FixedPoint(std::max(ta.int_bits(), tb.int_bits()) + 1 + std::max(ta.bits() - ta.int_bits(), tb.bits() - tb.int_bits()), std::max(ta.int_bits(), tb.int_bits()) + 1, std::max(ta.lanes(), tb.lanes()));
            //out_type = FixedPoint(max(ta.bits(), tb.bits())+1, max(ta.int_bits(), tb.int_bits())+1, max(ta.lanes(), tb.lanes()))
            //out_type = 
        } else if (ta.is_fixed_ufixed_point() && tb.is_fixed_ufixed_point()) {
...
struct Mul : public ExprNode<Mul> {
    ...
    inline Type match_output_type(const Expr &a, const Expr &b) {
        ...
        if (ta.is_fixed_ufixed_point() && tb.is_float()) {
            ...
            int bits_b;
            if (int_b) {
                bits_b = get_min_nb_bits(int_b->value >= 0 ? int_b->value : std::abs(int_b->value)+1);
            } else if (uint_b) {
                bits_b = get_min_nb_bits(uint_b->value);
            } else {
                bits_b = tb.bits();
            ...
            int bits_a;
            if (int_a) {
                bits_a = get_min_nb_bits(int_a->value >= 0 ? int_a->value : std::abs(int_a->value)+1);
            } else if (uint_a) {
                bits_a = get_min_nb_bits(uint_a->value);
            } else {
                bits_a = tb.bits();
