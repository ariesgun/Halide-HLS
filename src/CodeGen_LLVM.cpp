...
std::unique_ptr<llvm::Module> CodeGen_LLVM::compile(const Module &input) {
    ...
    if (input.target().has_feature(Target::Zynq)) {
        buffer_t_type = module->getTypeByName("struct.buffer_t");
    } else {
        buffer_t_type = module->getTypeByName("struct.halide_buffer_t");
    }
    internal_assert(buffer_t_type) << "Did not find halide_buffer_t or buffer_t in initial module";
...
void CodeGen_LLVM::compile_buffer(const Buffer<> &buf) {
    ...
    Constant *buffer_struct;

    if (input_module->target().has_feature(Target::Zynq)) {
        ...
        buffer_t old_buffer = buf.make_legacy_buffer_t();

        // Figure out the offset of the last pixel.
        size_t num_elems = 1;
        for (int d = 0; old_buffer.extent[d]; d++) {
            num_elems += old_buffer.stride[d] * (old_buffer.extent[d] - 1);
        }

        vector<char> array(old_buffer.host, old_buffer.host + num_elems * old_buffer.elem_size);

        Constant *fields[] = {
            ConstantInt::get(i64_t, 0), // dev
            create_binary_blob(array, buf.name() + ".data", constant), // host
            ConstantArray::get(i32_array, get_constants(i32_t, old_buffer.extent, old_buffer.extent + 4)),
            ConstantArray::get(i32_array, get_constants(i32_t, old_buffer.stride, old_buffer.stride + 4)),
            ConstantArray::get(i32_array, get_constants(i32_t, old_buffer.min, old_buffer.min + 4)),
            ConstantInt::get(i32_t, old_buffer.elem_size),
            ConstantInt::get(i8_t, 1), // host_dirty
            ConstantInt::get(i8_t, 0), // dev_dirty
            Constant::getNullValue(padding_bytes_type)
        };

        buffer_struct = ConstantStruct::get(buffer_t_type, fields);
