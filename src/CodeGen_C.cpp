...
void CodeGen_C::visit(const Call *op) {
    ...
    if (op->is_intrinsic(Call::debug_to_file)) {
        ...
        // #Aries add copy_buffer_t case
        debug(0) << "Apparently copy_buffer_t is called for op " << op->name << "\n";
        internal_assert(op->args.size() >= 2);
        vector<string> args;
        ...
        // #Aries add extract_buffer_host case
        debug(0) << "Apparently extract_buffer_host is called for op " << op->name << "\n";
        internal_assert(op->args.size() == 1);
        string a0 = print_expr(op->args[0]);
...
int test1_argv(void **args) HALIDE_FUNCTION_ATTRS {
 halide_buffer_t* _buf = *((halide_buffer_t**) args[0]);
