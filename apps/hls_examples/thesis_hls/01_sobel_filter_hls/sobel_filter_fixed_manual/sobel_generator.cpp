...
public:
	...
	void generate() {
        ...
        //conv1 = clamped;
        conv_x(x, y) += cast<uint16_t> (gray(x+win.x, y+win.y) * kernel_x(win.x + 1, win.y + 1));
        conv_y(x, y) += cast<uint16_t> (gray(x+win.x, y+win.y) * kernel_y(win.x + 1, win.y + 1));

        val = cast<uint16_t>(sqrt(( conv_x(x, y) * conv_x(x, y) + conv_y(x, y) * conv_y(x, y) )));
	...
	void type_schedule() {
		...
		// 	gray.cast_to(FixedPoint(20, 9), {{Expr(0.299f), FixedPoint(12, 1)}, {Expr(0.587f), FixedPoint(12, 1)}, {Expr(0.114f), FixedPoint(12,1)}});
		...
		// 	conv_y.cast_to(FixedPoint(23,11));
		// 	conv_x.cast_to(FixedPoint(23,11));
		// }	
	}
