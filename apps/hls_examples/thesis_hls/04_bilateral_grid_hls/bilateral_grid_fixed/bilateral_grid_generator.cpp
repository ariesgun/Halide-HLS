...
public:
    ...
    void generate() {
        ...
        // histogram(z, x, y, c) = 0.0f;
        // histogram(z, x, y, c) += select(c == 0, val, 1.0f);
        ...
        blurz(x, y, z, c) = (histogram(x, y, z-2, c) +
                             ...
                             histogram(x, y, z+1, c)*4 +
                             histogram(x, y, z+2, c));
        // blurz(x, y, z, c) = (histogram(z-2, x, y, c) +
        //                      histogram(z-1, x, y, c)*4 +
        //                      histogram(z  , x, y, c)*6 +
        //                      histogram(z+1, x, y, c)*4 +
        //                      histogram(z+2, x, y, c));
        ...
        zi      = cast<int>(zv);
        zf = zv - zi;
        xf = cast<float>(x % s_sigma) / s_sigma;
        yf = cast<float>(y % s_sigma) / s_sigma;
        Expr xi = x/s_sigma;
        Expr yi = y/s_sigma;
    ...
    void type_schedule() {
        ...
        if (get_target().has_hls_feature()) {
            clamped.cast_to(FixedPoint(18,2));
            input2.cast_to(FixedPoint(18,2));
            // clamped.cast_to(FixedPoint(10,2));
            // input2.cast_to(FixedPoint(10,2));

            histogram.cast_to(FixedPoint(24,8), {{test, FixedPoint(2,2)}}, {{val_const_1, FixedPoint(5,5)}, {val_const_2, FixedPoint(3,2)}, });
            // histogram.cast_to(FixedPoint(16,8), {{test, FixedPoint(2,2)}}, {{val_const_1, FixedPoint(5,5)}, {val_const_2, FixedPoint(3,2)}});

            // Blur applications.
            blurz.cast_to(FixedPoint(28,12));
            blurx.cast_to(FixedPoint(28,16));
            blury.cast_to(FixedPoint(28,20));

            // blurz.cast_to(FixedPoint(20,12));
            // blurx.cast_to(FixedPoint(22,16));
            // blury.cast_to(FixedPoint(24,20));

            interpolated.cast_to(FixedPoint(28,20), {{xf, UFixedPoint(9,1)}, {zf, UFixedPoint(9,1)}, {yf, UFixedPoint(9,1)}});
            bilateral_grid.cast_to(FixedPoint(10,2));
        }

    }

    Expr val_const_1, val_const_2, val_const_3;
    Expr zf, yf, xf;
    Expr test;
