...
public:
	...
	void generate() {
        ...
        output(x, y) = cast<float> (clamp(hw_output(x, y), 0, 1.0f));
	...
	void type_schedule() {
		...
		// 	//hw_output.cast_to(UInt(8));
			...
			gray.cast_to(FixedPoint(28, 2), {{R_const, FixedPoint(12, 2)}, {G_const, FixedPoint(12, 2)}, {B_const, FixedPoint(12,2)}});
			conv_y.cast_to(FixedPoint(28, 3));
			conv_x.cast_to(FixedPoint(28, 3));
