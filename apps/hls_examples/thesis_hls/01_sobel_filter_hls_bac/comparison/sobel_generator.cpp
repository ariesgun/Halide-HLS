...
public:
	...
	void generate() {
        ...
        gray(x, y) =  cast<int32_t>( (((77 *  cast<uint16_t>(clamped(x, y, 0))) + (150 *  cast<uint16_t>(clamped(x, y, 1))) + (29 *  cast<uint16_t>(clamped(x, y, 2)))) >> 8) / 255.0f * 65535);
        ...
        // hw_output(x, y) = cast<uint16_t>(gray(x, y));
