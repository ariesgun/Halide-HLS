...
public:
	...
	void type_schedule() {
		if (get_target().has_hls_feature()) {
			kernel.cast_to(UFixedPoint(25, 1));
			clamped.cast_to(UFixedPoint(20, 1));
			blur_x.cast_to(UFixedPoint(25, 1));
			blur_y.cast_to(UFixedPoint(25, 1));
			sharpen.cast_to(UFixedPoint(25, 1));
			hw_output.cast_to(UFixedPoint(26, 1));
		}		
	}
