...
public:
	...
	void type_schedule() {
		if (get_target().has_hls_feature()) {
			clamped.cast_to(UFixedPoint(8, 1));
			kernel.cast_to(UFixedPoint(8, 1));
			blur_x.cast_to(UFixedPoint(10, 1));
			blur_y.cast_to(UFixedPoint(10, 1));
			sharpen.cast_to(UFixedPoint(10, 1));
			hw_output.cast_to(UFixedPoint(10, 1));
		}		
	}
