...
public:
	...
	void type_schedule() {
		if (get_target().has_hls_feature()) {
			clamped.cast_to(UFixedPoint(16, 1));
			kernel.cast_to(UFixedPoint(20, 1));
			blur_y.cast_to(UFixedPoint(21, 1));
			blur_x.cast_to(UFixedPoint(21, 1));
			sharpen.cast_to(UFixedPoint(21, 1));
			hw_output.cast_to(UFixedPoint(22, 1));
		}		
	}
