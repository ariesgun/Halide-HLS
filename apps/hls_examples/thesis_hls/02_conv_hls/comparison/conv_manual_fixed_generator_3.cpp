...
class GaussianConv3: public Halide::Generator<GaussianConv3> {
...
public:
	...
	void generate() {
        ...
        //conv1 = clamped;
        conv1(x, y) += cast<uint16_t>(clamped(x, y)) * kernel(win.x, win.y);
        conv1_shifted(x, y) = cast<uint8_t> (conv1(x, y) >> 8);
...
HALIDE_REGISTER_GENERATOR(GaussianConv3, "gaussian_conv_3");
