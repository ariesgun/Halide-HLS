...
public:
	...
	void generate() {
        ...
        //gray(x, y) =  cast<int>(((77 *  cast<int>(clamped(x, y, 0))) + (150 *  cast<int>(clamped(x, y, 1))) + (29 *  cast<int>(clamped(x, y, 2)))) >> 8);

        conv_x(x, y) = ( (-1 * cast<int>(clamped(x-1, y-1))) + (1 * cast<int>(clamped(x+1, y-1)))  + 
        				 (-2 * cast<int>(clamped(x-1, y)))   + (2 * cast<int>(clamped(x+1, y)))    + 
        				 (-1 * cast<int>(clamped(x-1, y+1))) + (1 * cast<int>(clamped(x+1, y+1))) ); 
        conv_y(x, y) = ( (-1 * cast<int>(clamped(x-1, y-1))) + (-2 * cast<int>(clamped(x, y-1))) + (-1 * cast<int>(clamped(x+1, y-1)))  + 
        				 (1  * cast<int>(clamped(x-1, y+1))) + (2 * cast<int>(clamped(x, y+1)))  + (1 * cast<int>(clamped(x+1, y+1))) ); 

        val = abs(conv_x(x, y)) + abs(conv_y(x, y));
        hw_output(x, y) = cast<int>(clamp(val, 0, 255));
	...
	void schedule() {
		...
		if (get_target().has_hls_feature()) {
	        ...
	        hw_output.tile(x, y, xo, yo, xi, yi, 960, 960).reorder(xi, yi, xo, yo);
